...
The public Vitest + Testcontainers example lives in `examples/vitest-testcontainers`.

It is validated separately from the package test suite by the dedicated `Examples` GitHub Actions job.

Local commands from `packages/integresql`:
